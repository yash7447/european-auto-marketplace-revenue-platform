...
USING (
    ...
    SELECT
            ...
            md5(
                to_utf8(
                    concat(
                        ...
                        k.market, '|', k.sales_rep, '|', k.product_group
                    )
                )
            )
        ) AS performance_key,
        ...
        COALESCE(tg.revenue_target_eur, DECIMAL '0.00') AS revenue_target_eur,
        COALESCE(tg.upsell_target_eur, DECIMAL '0.00') AS upsell_target_eur,
        COALESCE(tg.new_dealer_target, 0) AS new_dealer_target,
        COALESCE(tg.retention_target_pct, DECIMAL '0.0000') AS retention_target_pct,
        CAST(
            CASE
                WHEN COALESCE(tg.revenue_target_eur, DECIMAL '0.00') = DECIMAL '0.00'
                    THEN NULL
                ELSE 100.0 * COALESCE(r.revenue_eur, DECIMAL '0.00')
                    / tg.revenue_target_eur
            END
            AS DECIMAL(12,4)
        ) AS attainment_pct,
        COALESCE(r.invoice_count, 0) AS invoice_count,
        COALESCE(r.revenue_dealer_count, 0) AS revenue_dealer_count,
...
WHEN NOT MATCHED THEN INSERT (
    performance_key, month_start, market, sales_rep, product_group,
    revenue_eur, revenue_target_eur, upsell_target_eur, new_dealer_target,
    retention_target_pct, attainment_pct, invoice_count, revenue_dealer_count,
    active_subscriptions, refreshed_at
) VALUES (
    s.performance_key, s.month_start, s.market, s.sales_rep, s.product_group,
    s.revenue_eur, s.revenue_target_eur, s.upsell_target_eur,
    s.new_dealer_target, s.retention_target_pct, s.attainment_pct,
    s.invoice_count, s.revenue_dealer_count, s.active_subscriptions,
    s.refreshed_at
);
